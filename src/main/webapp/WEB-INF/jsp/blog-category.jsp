        ...
        <main class="posts-listing col-lg-8"> 
          <div class="container">
            ...
            <c:if test="${currentPage1>1}">
                <li class="page-item"><a href="${pageContext.request.contextPath}/blog-category/${category.ceoTitle}/${category.id}/${currentPage1-1}" class="page-link"> <i class="fa fa-angle-left">
                
                </i></a></li></c:if>
                   <c:forEach var="p" begin="1" end="${totalPage}">
                   <c:choose><c:when test="${p==currentPage1}">
                <li class="page-item"><a href="${pageContext.request.contextPath}/blog-category/${category.ceoTitle}/${category.id}/${p}" class="page-link active">${p}</a></li>
             
            </c:when>
            <c:otherwise>
             <li class="page-item"><a href="${pageContext.request.contextPath}/blog-category/${category.ceoTitle}/${category.id}/${p}" class="page-link">${p}</a></li>
            </c:otherwise> </c:choose>
               </c:forEach>
               <c:if test="${currentPage1<totalPage}">
                <li class="page-item"><a href="${pageContext.request.contextPath}/blog-category/${category.ceoTitle}/${category.id}/${currentPage1+1}" class="page-link"> <i class="fa fa-angle-right"></i></a></li> </c:if>
              </ul>
            </nav>
          </div>
        </main>
